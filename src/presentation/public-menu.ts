...
import { env } from '@/env';
import { carregarMenuPublico, type MenuPublico } from '@/infrastructure/catalog/menu-publico';
...
export async function getMenuPublico(slug: string): Promise<MenuPublico | null> {
  return carregarMenuPublico(slug);
}
...
export async function localizarEnderecoAction(
  ...
  const ponto = await localizarEnderecoDaLoja(slug, endereco);
  return ponto ? { ok: true, lat: ponto.lat, lng: ponto.lng } : { ok: false };
}
